...
public class CharacterEventStore implements CharacterRepository {
...
//    private final CharacterProjector characterManager;
...
//    }
    ...
    public List<PlayerCharacter> getAllCharacters() {
        try (Consumer<String, DomainEvent> consumer = createConsumer()) {
            ...
            Map<String, List<DomainEvent>> eventsPerCharacter = new HashMap<>();
            ...
            while (true) {
                ...
                if (records.isEmpty()) {
                    ...
                    for (ConsumerRecord<String, DomainEvent> record : records) {
                        ...
                        if (!eventsPerCharacter.containsKey(characterName)) {
                            eventsPerCharacter.put(characterName, new ArrayList<>());
                        } else {
                            eventsPerCharacter.get(characterName).add(event);
                        }
                    }
                }
            }

            for (Map.Entry<String, List<DomainEvent>> entry : eventsPerCharacter.entrySet()) {
                characters.add(replayEvents(entry.getValue()));
            }
